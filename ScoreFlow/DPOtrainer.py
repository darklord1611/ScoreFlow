...
from ScoreFlow.DPOconfig import DPOConfig, FDivergenceConstants, FDivergenceType
...
class PreferenceCollator(DataCollatorMixin):
    ...
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        if "chosen_score" in examples[0]:
            chosen_score = torch.tensor([torch.tensor(example["chosen_score"]) for example in examples])
            rejected_score = torch.tensor([torch.tensor(example["rejected_score"]) for example in examples])
            output["chosen_score"] = chosen_score
            output["rejected_score"] = rejected_score
...
class DPOTrainer(Trainer):
    ...
    def tokenize_row(self, features, processing_class, max_prompt_length, max_completion_length, add_special_tokens):
        ...
        if self.use_score == True:
            output["chosen_score"] = features["chosen_score"]
            output["rejected_score"] = features["rejected_score"]
    ...
    def _set_signature_columns_if_needed(self):
        ...
        if self._signature_columns is None:
            if self.use_score == True:
                self._signature_columns = ["prompt_input_ids", "chosen_input_ids", "rejected_input_ids", "chosen_score", "rejected_score"]
            else:
    ...
    def get_batch_loss_metrics(
        ...
        if self.use_score == True:
            ...
            losses, chosen_rewards, rejected_rewards = self.dpo_loss(
                model_output["chosen_logps"]*chosen_score, model_output["rejected_logps"]*(1 - rejected_score), ref_chosen_logps*chosen_score, ref_rejected_logps*(1 - rejected_score)
            )
